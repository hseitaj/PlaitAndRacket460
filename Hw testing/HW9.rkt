...
;; Apologies in advance for the hackiness I resorted to in a couple places :\
;; Dan Ruley          u0956834
;; CS3520             Assignment 9
    ...
    (local [(define (recur expr)
      ...
      (type-case Exp a
        ...
        [(selectE cond-expr obj-expr)
         (type-case Value (recur cond-expr)
           [(numV n)
            (type-case Value (recur obj-expr)
              [(objV class-name field-vals)
               (type-case Class (find classes class-name)
                 [(classC field-names methods supers)
                  (if (zero? n)
                      (local [(define check (find methods 'zero))]
                        (recur (sendE obj-expr 'zero (numE 0))))
                      (local [(define check (find methods 'nonzero))]
                        (recur (sendE obj-expr 'nonzero (numE 0)))))])]
              [else (error 'interp "not an object")])]
           [else (error 'interp "not a number")])]
        [(instanceofE obj-expr super)
         (type-case Value (recur obj-expr)
           [(objV class-name field-vals)
            (if (symbol=? super 'Object)
                (numV 0)
                (try
                 (type-case Class (find classes super)
                   [(classC field-names methods supers)
                    (type-case Class (find classes class-name)
                      [(classC field-names methods supers)
                       (if (or (symbol=? super class-name) (member super supers))
                           (numV 0)
                           ;;I tried to construct an example to test this condition, but could not get it to work correctly
                           (numV 1))])]) (lambda () (numV 1))))]
           [else (error 'interp "not an object")])]
...
(module+ test
  ...
  (test/exn (interp-prog (list `{class Snowball extends Object
                              ...
                              [nonzero {arg}
                                       {new Snowball {+ 1 {get this size}}}]})
                     `{get {select {new Object} {new Snowball 1}} size})
        "not a number")

  
  (test/exn (interp-prog empty `{select {+ 1 2} {* 5 6}})
        "not an object")
    ...
    (values 'Posn
            (classC 
             ...
             (list (values 'mdist
                   ...
                   (values 'factory12 (newE 'Posn (list (numE 1) (numE 2)))))empty)))
    ...
    (values 'Posn3D
            (classC 
             ...
             (list (values 'mdist (plusE (getE (thisE) 'z)
                   ...
                   (values 'addDist (ssendE (thisE) 'Posn 'addDist (argE)))) empty)))
...
(module+ test
  ...
  (test/exn (interp (instanceofE (plusE (numE 5) (numE 6)) 'x) empty (objV 'Object empty) (numV 0)) 
            "not an object")
...
(define-type ClassI
  (classI [super-names : (Listof Symbol)]
  ...
  (local [(define (recur expr)
    ...
    (type-case ExpI a
      ...
      [(selectI cond-expr obj-expr)
       (selectE (recur cond-expr) (recur obj-expr))]
      [(instanceofI obj-expr super)
       (instanceofE (recur obj-expr) super)]
...
(define (class-i->c-not-flat [c : ClassI]) : Class
  (type-case ClassI c
    [(classI super-names field-names methods)
     (classC
      field-names
      (map (lambda (m)
             (values (fst m)
                     (exp-i->c (snd m) (first super-names))))
           methods) super-names)]))
  ...
  (type-case Class (find classes-not-flat name)
    [(classC field-names methods supers)
     (type-case Class (flatten-super name classes-not-flat i-classes)
       [(classC super-field-names super-methods super-supers)
        (classC
         (add-fields super-field-names field-names)
         (add/replace-methods super-methods methods)
         (cons (first supers) super-supers))])]))

(define (flatten-super [name : Symbol]
                       [classes-not-flat : (Listof (Symbol * Class))] 
                       [i-classes : (Listof (Symbol * ClassI))]) : Class
  (type-case ClassI (find i-classes name)
    [(classI super-names field-names i-methods)
     (if (equal? (first super-names) 'Object)
         (classC empty empty empty)
         (flatten-class (first super-names)
                        classes-not-flat
                        i-classes))]))
  ...
  (cond
    [(s-exp-match? `{class SYMBOL extends SYMBOL {ANY ...} ANY ...} s)
     (values (s-exp->symbol (second (s-exp->list s)))
             (classI (list (s-exp->symbol (fourth (s-exp->list s))))
                     (map parse-field
