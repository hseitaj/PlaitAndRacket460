...
(define-type Value
  ...
  (closV [arg : Symbol]
         [body : Exp]
         [env : Env])
  ;use Thunks so lazy evaluation of pairs is supported
  (pairV [fst : Thunk]
         [snd : Thunk]))
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{pair ANY ANY} s) (pair
                                       (parse (second (s-exp->list s)))
                                       (parse (third (s-exp->list s))))]
    [(s-exp-match? `{fst ANY} s) (fst (parse (second (s-exp->list s))))]
    [(s-exp-match? `{snd ANY} s) (snd (parse (second (s-exp->list s))))]
    [(s-exp-match? `{ANY ANY} s)
...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(pair fst snd) (pairV
                     (delay fst env (box (none)))
                     (delay snd env (box (none))))]
    [(fst p) (type-case Value (interp p env)
                       [(pairV f s) (force f)]
                       [else (error 'interp "not a pair")])]
    [(snd p) (type-case Value (interp p env)
                       [(pairV f s) (force s)]
                       [else (error 'interp "not a pair")])]
    [(if0 cond true false) (interp (if (num-zero? (interp cond env))
                                       true
                                       false)
                                   env)]
...
(module+ test
  ...
  (test (interp-expr (parse `{lambda {x} {+ x x}}))
        `function)
  (test/exn (interp-expr (parse `{fst {+ 1 3}}))
            "not a pair")
  (test/exn (interp-expr (parse `{snd {+ 1 3}}))
            "not a pair")
  (test/exn (interp-expr (parse `{if0 {pair 1 2} 1 2}))
            "not a number")
