...
(module+ test
  ...
  (test (parse `{first {cons 1 empty}})
        (firstE (consE (numE 1) (emptyE))))
  (test (parse `{rest {cons 1 {cons 2 empty}}})
        (restE (consE (numE 1) (consE (numE 2) (emptyE))))))
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{if0 ANY ANY ANY} s)
     (let ([expL (map parse (rest (s-exp->list s)))])
       (if0E (first expL) (second expL) (third expL)))]
...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(if0E cond pass fail) (if (= 0 (numV-n (interp cond env)))
                               (interp pass env)
                               (interp fail env))]
    [(lamE n t body)
     (closV n body env)]
...
(module+ test
  ...
  (test (lookup 'y (extend-env
        ...
        (numV 8))
  (test/exn (typecheck (parse `{if0 0 {lambda {[x : ?]} x} 2}) mt-env)
            "no type")
...
(define (typecheck [a : Exp] [tenv : Type-Env])
  (type-case Exp a
    ...
    [(if0E cond pass fail) (begin (unify! (typecheck cond tenv) (numT) cond)
                                  (let ([pType (typecheck pass tenv)])
                                    (begin
                                      (unify! (typecheck fail tenv) pType fail)
                                      pType)))]
    ...
    [(consE l r) (let ([lt (typecheck l tenv)])
                   (begin (unify! (listofT lt) (typecheck r tenv) l)
                        (listofT lt)))]
    [(firstE a) (let ([resT (varT (box (none)))])
                 (begin
                   (unify! (typecheck a tenv) (listofT resT) a)
                   resT))]
    [(restE a) (let ([resT (varT (box (none)))])
                 (begin
                   (unify! (typecheck a tenv) (listofT resT) a)
                   (listofT resT)))]))
...
(module+ test
  ;Tests for typecheck and run-prog for lists
  (test (typecheck (parse `{cons {lambda {[x : num]} x} empty}) empty)
        (listofT (arrowT (numT) (numT))))
  (test (resolve (typecheck (parse `{first {cons {lambda {[x : num]} x} empty}}) mt-env))
        (arrowT (numT) (numT)))
  (test (typecheck (parse `{rest {cons 1 {cons 2 {cons 3 empty}}}}) mt-env)
        (listofT (varT (box (some (numT))))))
  (test (typecheck (parse `{rest empty}) mt-env)
        (typecheck (parse `empty) mt-env))
...
;;run-prog ---------------------------------------
(define (run-prog [exp : S-Exp]) : S-Exp
  (let ([parsedExp (parse exp)])
    (begin (typecheck parsedExp mt-env)
           (type-case Value (interp parsedExp mt-env)
...
;;If0 tests
(module+ test
  (test/exn (resolve (typecheck (parse `{if0 {{lambda {[x : ?]} {+ x 1}} -1} 1 {lambda {[x : ?]} x}}) mt-env))
        "no type")
  (test (resolve (typecheck (parse `{if0 {{lambda {[x : ?]} {+ x 1}} -1} 1 2}) mt-env))
        (numT))
  (test (interp (parse `{if0 {{lambda {[x : ?]} {+ x 1}} -1} 1 2}) mt-env)
        (numV 1))
  (test (interp (parse `{if0 {{lambda {[x : ?]} {+ x 1}} 1} 1 2}) mt-env)
        (numV 2))
  ...
  (type-case Type t1
    ...
    [else
     (type-case Type t2
       ...
       [(listofT e2)
        (type-case Type t1
        [(listofT elem) (unify! elem e2 expr)]
        [else (type-error expr t1 t2)])])]))
