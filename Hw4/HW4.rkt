#lang plait
; Hansi Seitaj, Hw4, CMPSC 460
; I tried part 3 and 4
...
;(define override-store cons) - fixed
...
(module+ test
  ...
  (test (parse `{begin 1 2})
        (beginE (list (numE 1) (numE 2)))) ; fixed
  (test/exn (parse `{{+ 1 2}})
            "invalid input"))
...
;#|
; Iterp begin helper function
(define (interp-begin-args [args : (Listof Exp)] [env : Env] [sto : Store]) : Result
  (type-case (Listof Exp) args
    [(cons arg rest)
     (cond [(empty? rest) (interp arg env sto)]
           [else (with [(v-arg sto-arg) (interp arg env sto)]
                       (interp-begin-args rest env sto-arg))])]
    [empty (error 'interp-begin "error - should not be here")]))

;Updated implementation of override-store: Looks for a cell with the same location
;already in storage - if so, replace it.
;Otherwise, cons it into the Store.
;; override-store --------------------------------
...
(module+ test
  ...
  (test/exn (interp (parse `{get 1 x}) mt-env mt-store)
            "not a record")
  (test (interp (parse `{+ 1 4}) mt-env mt-store)
        (v*s (numV 5) '()))
