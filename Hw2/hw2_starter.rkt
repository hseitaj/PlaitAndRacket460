...
(define (parse-fundef [s : S-Exp]) : Func-Defn
  (cond
    [(s-exp-match? `{define {SYMBOL [SYMBOL]} EXP} s)
     (let* ([s-args (second (s-exp->list (second (s-exp->list s))))] ; new
            [args (map s-exp->symbol s-args)])                       ; new
       (fd (s-exp->symbol (second (s-exp->list s)))
           args
           (parse (third (s-exp->list s)))))]
    [else (error 'parse-fundef "wrong arity")]))
...
(module+ test
  ...
  (test (parse `{+ {* 3 4} 8})
        (plusE (multE (numE 3) (numE 4))
               (numE 8)))
  ;(test (parse `{double 9})
   ;     (appE 'double (numE 9)))  throwing error
  (test/exn (parse `{{+ 1 2}})
            "invalid input")
...
(define (interp [a : Exp] [defs : (Listof Func-Defn)]) : Number
  (type-case Exp a
    ...
    [(appE s arg) (local [(define fd (parse-fundef s))] ; ?????
                    (interp (subst (numE (interp (second arg) defs)) ; value
                                   (fd-arg fd)                  ;name
                                   (fd-body fd))              ;body
                            defs))]
                  ;(else (error 'parse-fundef "wrong arrity"))]
    [(maxE l r) (max (interp l defs) (interp r defs) )])) ; one too many interps
...
(define (subst [what : Exp] [for : Symbol] [in : Exp])
  (type-case Exp in
    ...
    [(maxE l r) (maxE (subst what for l)
                      (subst what for r))]
    [(appE s arg) (appE s (subst what for (first (first (arg)))))])) ;???
                      ;(subst what for (third arg)))]))
